...
def test(args):
    if args.model == "EDSR":
        ...
        if args.which_sar == "complex":
            model = EDSR(args)
            model.load_state_dict(torch.load(os.path.join(args.model_path, "MST_AFRL_ps256rb8_qp21_complex_best.pt")))
            model.to(device)
            print("total number of parameters (complex): ", sum(p.numel() for p in model.parameters()))
            model.eval()
        else:
    ...
    with torch.no_grad():
        #pdb.set_trace()
        ori_sar = np.load('/home/pmc4p/PythonDir/dataset/SAR_dataset/nonuniform/AFRL_nonuniform_SAR_HEVC_ps256qp%d_test/gt/0.npy'%qp)
        ...
        rec_sar = np.load('/home/pmc4p/PythonDir/dataset/SAR_dataset/nonuniform/AFRL_nonuniform_SAR_HEVC_ps256qp%d_test/input/0.npy'%qp)
        ...
        amp_img = np.load('/home/pmc4p/PythonDir/dataset/SAR_dataset/nonuniform/AFRL_nonuniform_SAR_HEVC_ps256qp%d_test/amp/0.npy'%qp)
        amp_img = amp_img/amp_max_val
        print("GT Amp", amp_img.shape, amp_img.min(), amp_img.max())

        pha_img = np.load('/home/pmc4p/PythonDir/dataset/SAR_dataset/nonuniform/AFRL_nonuniform_SAR_HEVC_ps256qp%d_test/phase/0.npy'%qp)
