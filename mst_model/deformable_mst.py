...
class MSTFusionBlock(nn.Module):
    def __init__(self, dim_imgfeat, dim_dctfeat, kernel_size=3):
        ...
        #self.conv_downscale = nn.Conv2d(dim_dctfeat, dim_dctfeat, kernel_size=kernel_size, stride=2,padding=(kernel_size//2))
    ...
    def forward(self, in_pix, in_dct):    
        ...
        #out_pix = self.conv_downscale(out_pix)
...
class EDSR(nn.Module):
    def __init__(self, args, conv=default_conv):
        ...
        #conv = nn.Conv2d if args.deformable==False else DeformableConv2d
...
if __name__=="__main__":
    import numpy as np
    import sys
    sys.path.append("/home/pmc4p/PythonDir/SAR-HEVC-Deblocking/src")
    from option import args
    model = EDSR(args).cuda()
    print(model)
    #model = DeformableConv2d(32,32,3).cuda()
    img = torch.tensor(torch.ones(1,2,128,128)).cuda()
    print(img.shape)
    out = model(img)
    print(out.shape)
    print("Done...")