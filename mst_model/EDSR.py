...
class EDSR(nn.Module):
    def __init__(self, args, conv=default_conv):
        ...
        self.head1 = nn.Sequential(nn.Conv2d(in_channels=in_channel1, out_channels=dim_x1//2, kernel_size=kernel_size, padding=(kernel_size//2), stride=1),
                                nn.PReLU(dim_x1//2),
                                nn.Conv2d(in_channels=dim_x1//2, out_channels=dim_x1, kernel_size=kernel_size, padding=(kernel_size//2), stride=1)
                                )
        
        self.head2 = nn.Sequential(nn.Conv2d(in_channels=in_channel2, out_channels=dim_x2//2, kernel_size=kernel_size, padding=(kernel_size//2), stride=1),
                               nn.PReLU(dim_x2//2),
                               nn.Conv2d(in_channels=dim_x2//2, out_channels=dim_x2, kernel_size=kernel_size, padding=(kernel_size//2), stride=1)
                               )
        
        self.body = nn.ModuleList([ MSTFusionBlock(dim_x1, dim_x2, kernel_size) for _ in range(n_basicblock) ])
        
        
        # define tail module
        self.tail = conv(dim_x1, out_channel, kernel_size)
...
if __name__=="__main__":
    ...
    # img = torch.tensor(torch.ones(32,2,128,128)).cuda()
    # print(img.shape)
    # out = model(img)
    # print(out.shape)
    print("Done...")